...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    This public RT-DETR-GMI release keeps the dynamic YAML resolver intentionally narrow:
    it supports upstream Ultralytics layers needed by RT-DETR plus the GMI modules used
    by ``RT-DETR-GMI.yaml``. Unrelated experimental modules are not registered here.
    """
    import ast
    ...
    ch = [ch]
    layers, save, c2 = [], [], ch[-1]
    repeat_modules = (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, C2f_MambaOut)
    channel_modules = (
        ...
        ConvNormLayer, C2f_MambaOut)
    transformer_modules = (AIFI, TransformerEncoderLayer_MSLA)
