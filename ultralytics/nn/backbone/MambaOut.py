# Derived from yuweihao/MambaOut (Apache-2.0).
# Modified for RT-DETR-GMI.
"""
...
class GatedCNNBlock_BCHW(nn.Module):
    r""" 我们对门控卷积神经网络模块的实现： https://arxiv.org/pdf/1612.08083
    参数： 
        conv_ratio：控制进行深度卷积的通道数。
            对部分通道进行卷积可以提高实际效率。
            部分通道的想法源自 ShuffleNet V2 (https://arxiv.org/abs/1807.11164) and 
            also used by InceptionNeXt (https://arxiv.org/abs/2303.16900) and FasterNet (https://arxiv.org/abs/2303.03667)
    """
    def __init__(self, dim, expansion_ratio=8/3, kernel_size=7, conv_ratio=1.0,
...
r"""
第一阶段的下采样（主干）是两层卷积层，卷积核大小为 3，步长为 2，填充为 1。
最后三个阶段的下采样是一层卷积层，卷积核大小为 3，步长为 2，填充为 1。
四阶段下采样层的格式为：[下采样，下采样，下采样，下采样]
使用 partial 来指定一些参数
"""
