...
def get_params():
    parser = argparse.ArgumentParser(description='Generate RT-DETR-GMI Grad-CAM heatmaps.')
    parser.add_argument('--weight', required=True, help='Trained checkpoint path.')
    parser.add_argument('--source', required=True, help='Input image path or image directory.')
    parser.add_argument('--output', default='runs/heatmap', help='Output directory.')
    parser.add_argument('--device', default='cuda:0', help="Device, for example 'cuda:0' or 'cpu'.")
    parser.add_argument('--method', default='GradCAMPlusPlus',
                        choices=['GradCAMPlusPlus', 'GradCAM', 'XGradCAM', 'EigenCAM', 'HiResCAM',
                                 'LayerCAM', 'RandomCAM', 'EigenGradCAM'],
                        help='CAM method.')
    parser.add_argument('--layer', default='15,19,22,25', help='Comma-separated target layer indexes.')
    parser.add_argument('--backward-type', default='all', choices=['class', 'box', 'all'], help='Backward target type.')
    parser.add_argument('--conf-threshold', type=float, default=0.5, help='Confidence threshold.')
    parser.add_argument('--ratio', type=float, default=1.0, help='Ratio of predictions used for CAM backward target.')
    parser.add_argument('--show-box', action=argparse.BooleanOptionalAction, default=True, help='Draw predicted boxes.')
    parser.add_argument('--renormalize', action=argparse.BooleanOptionalAction, default=True,
                        help='Restrict heatmap normalization to predicted boxes.')
    ...
    return {
        ...
        'device': args.device,
        'method': args.method,
        'layer': [int(x.strip()) for x in args.layer.split(',') if x.strip()],
